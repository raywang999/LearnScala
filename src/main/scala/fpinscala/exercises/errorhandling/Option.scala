...
object Option:
  ...
  def map2[A,B,C](a: Option[A], b: Option[B])(f: (A, B) => C): Option[C] = 
    //a.flatMap(a => b.map(b => f(a,b))) // wait why does this pass? 
    a.flatMap(aa => b.map(bb => f(aa,bb))) // so almost official solution

  def sequence[A](as: List[Option[A]]): Option[List[A]] = 
    as.foldRight(Some(Nil: List[A]))((cur, res) => map2(cur, res)((x, xs) => x::xs))
